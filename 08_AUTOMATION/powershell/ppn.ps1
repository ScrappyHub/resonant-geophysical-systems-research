...
switch ($Command) {
  ...
  'resonance-plot' {
    ...
    # Copy plots + best_band into reports folder too
    Get-ChildItem $dir -Filter *.png -File | ForEach-Object { Copy-Item [CmdletBinding()]
param(
  [Parameter(Mandatory=$true, Position=0)]
  [ValidateSet('where','status','init-run','ingest','quicklook','resonance','resonance-summary','resonance-plot')]
  [string]$Command,
...
.FullName $reportDir -Force }
    $band = Join-Path $dir "best_band.txt"
    if (Test-Path $band) { Copy-Item $band $reportDir -Force }
