...
# ----------------------------
# README SECTION (managed, NO REGEX)
# ----------------------------
...
if (Test-Path -LiteralPath $ReadmePath) {
  ...
  $r = "# Plantery Pyramid Network`r`n"
}

$startIdx = $r.IndexOf($sectionStart, [StringComparison]::Ordinal)
$endIdx   = $r.IndexOf($sectionEnd,   [StringComparison]::Ordinal)
if ($startIdx -ge 0 -and $endIdx -ge 0 -and $endIdx -ge $startIdx) {
  $endIdx2 = $endIdx + $sectionEnd.Length
  $before = $r.Substring(0, $startIdx)
  $after  = $r.Substring($endIdx2)
  $r = ($before.TrimEnd() + "`r`n`r`n" + $readmeSection + "`r`n`r`n" + $after.TrimStart())
} else {
...
# ----------------------------
# Optional TEMP cleanup patch (safe; no $_ expansion at generation time)
# ----------------------------
