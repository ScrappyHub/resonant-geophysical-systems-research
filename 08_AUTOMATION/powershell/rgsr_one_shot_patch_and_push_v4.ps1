...
# ==========================================================
# C) REWRITE ALL drive_profiles_ui... migrations to canonical SQL
#    (fixes the broken 20260101194411 delimiter issue)
# ==========================================================
$driveTargets = Get-ChildItem -LiteralPath $mgDir -Filter "*_rgsr_drive_profiles_ui_and_institution_timeline_v1.sql" |
  Sort-Object Name

if (-not $driveTargets -or $driveTargets.Count -eq 0) {
  throw "No drive UI migrations found to rewrite: *_rgsr_drive_profiles_ui_and_institution_timeline_v1.sql"
}
...
foreach($t in $driveTargets){
  WriteUtf8NoBomIfChanged $t.FullName $canonicalDriveSql
  Write-Host ("[OK] REWROTE DRIVE MIGRATION (canonical): " + $t.FullName) -ForegroundColor Green
}
...
Write-Host "✅ ONE-SHOT PIPELINE COMPLETE (v4)" -ForegroundColor Green