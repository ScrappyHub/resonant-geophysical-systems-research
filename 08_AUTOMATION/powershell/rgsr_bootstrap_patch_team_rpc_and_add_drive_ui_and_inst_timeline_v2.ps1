...
# Correct SQL function body (single here-string = no escaping games)
$replacement = @"
...
-- ------------------------------------------------------------
-- 0) Temp conversion helpers (canonical, pure SQL)
-- ------------------------------------------------------------
create or replace function rgsr.c_to_f(p_c numeric) returns numeric language sql immutable as $$
  select (p_c * 9.0/5.0) + 32.0;
$$;

create or replace function rgsr.f_to_c(p_f numeric) returns numeric language sql immutable as $$
  select (p_f - 32.0) * 5.0/9.0;
$$;

-- ------------------------------------------------------------
-- 1) Drive Profiles UI RPC: returns computed drive_json using user temp_unit
-- ------------------------------------------------------------
...
begin
  ...
  -- Units annotation (non-destructive)
  ...
  -- Known optional paths (future-proof):
...
-- ------------------------------------------------------------
-- 2) Institution Application Event Log (canonical timeline)
-- ------------------------------------------------------------
...
WriteUtf8NoBomIfChanged $mgPath ($sql + "`r`n")
