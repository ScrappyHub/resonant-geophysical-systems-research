...
function Invoke-Supabase([string[]]$Args, [switch]$PipeYes) {
  $argStr = ($Args -join " ")
  if ($PipeYes) {
...
$mgPath = Join-Path $mgDir ("{0}_rgsr_runs_and_measurements_v10_1_fix.sql" -f $MigrationId)
...
if ($ApplyRemote) {
  Invoke-Supabase -Args @("db","push") -PipeYes
  Write-Host "[OK] supabase db push complete" -ForegroundColor Green
}

Write-Host "✅ ONE-SHOT PIPELINE COMPLETE (v10.1 FIX runs+measurements+labs)" -ForegroundColor Green