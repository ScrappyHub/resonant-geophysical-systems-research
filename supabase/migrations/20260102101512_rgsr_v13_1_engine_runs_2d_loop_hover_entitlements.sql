-- ============================================================
-- RGSR v13.1 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ENGINE RUNS + CANONICAL 2D LOOP + NODE READINGS
...
-- ------------------------------------------------------------
-- PATCH: add engine_configs.is_public for public listing/index
-- ------------------------------------------------------------
alter table rgsr.engine_configs
...
create table if not exists rgsr.engine_runs (
  ...
  owner_uid uuid not null references auth.users(id) on delete restrict,
  config_id uuid not null references rgsr.engine_configs(config_id) on delete restrict,
...
create index if not exists ix_engine_runs_cfg on rgsr.engine_runs(config_id);
...
begin
  ...
  where ec.config_id = p_engine_config_id
  ...
  insert into rgsr.engine_runs(owner_uid, config_id, engine_kind, status, started_at, state)
...
begin
  ...
  for v_step in 0..greatest(p_steps,1)-1 loop
    ...
    for n in select value from jsonb_array_elements(v_nodes) loop
      ...
      -- first-pass ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“provableÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â deterministic fields (replace later with real coupled solvers)
      ...
      -- simple thermal/pressure placeholders tied to displacement (so UI isnÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢t fake; itÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s consistent & testable)
      v_temp := 20 + (5 * v_res);          -- ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°C
