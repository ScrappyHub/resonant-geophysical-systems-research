...
-- - Materials: required physical properties (non-negotiable)
-- - Layer stacks (ordered, material-referenced)
-- - Water-stone interactions
-- - Subsurface features
-- - Geometry<->Geology coupling mappings
-- - Conservative RLS (read authenticated; write sys_admin/service_role)
-- ============================================================
...
create table if not exists rgsr.material_profiles (
  ...
  classification text not null default 'STONE',
  ...
  grain_structure text not null,
  crystalline_order text not null,
  ...
  piezoelectric_potential_index numeric not null,
  thermal_inertia_j_m2_k_s05 numeric not null,
  ...
  constraint material_profiles_density_chk check (density_kg_m3 > 0),
  constraint material_profiles_youngs_chk check (youngs_modulus_pa > 0),
  constraint material_profiles_poisson_chk check (poissons_ratio >= 0 and poissons_ratio < 0.5),
  constraint material_profiles_impedance_chk check (acoustic_impedance_rayl > 0),
  constraint material_profiles_porosity_chk check (porosity_frac >= 0 and porosity_frac <= 1),
  constraint material_profiles_moisture_chk check (moisture_content_frac >= 0 and moisture_content_frac <= 1),
  constraint material_profiles_sigma_chk check (electrical_conductivity_s_m >= 0),
  constraint material_profiles_k_chk check (thermal_conductivity_w_mk > 0),
  constraint material_profiles_eps_chk check (dielectric_constant_rel > 0),
  constraint material_profiles_piezo_chk check (piezoelectric_potential_index >= 0 and piezoelectric_potential_index <= 1),
  constraint material_profiles_thermin_chk check (thermal_inertia_j_m2_k_s05 > 0)
);
...
create table if not exists rgsr.geology_structures (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint geology_structures_lane_chk check (lane in ('PUBLIC','LAB','INTERNAL')),
  constraint geology_structures_type_chk check (structure_type in ('CHAMBER','WALL','SHAFT','CASING','SUBSTRATE','FOUNDATION','OTHER'))
);
...
create table if not exists rgsr.geology_layers (
  ...
  contact_impedance_rayl numeric not null,
  moisture_frac numeric not null,
  thermal_gradient_c_per_m numeric not null,
  electrical_coupling_index numeric not null,
  notes text,
...
create unique index if not exists uq_geology_layers_structure_order on rgsr.geology_layers(structure_id, layer_order);
...
create table if not exists rgsr.water_stone_interactions (
  ...
  absorption_frac numeric not null,
  capillary_flow_index numeric not null,
  surface_wetting_index numeric not null,
  dissolved_mineral_ppm numeric not null,
  conductivity_delta_s_m numeric not null,
  temp_coupling_index numeric not null,
  notes text,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists rgsr.subsurface_features (
  ...
  coupling_index numeric not null default 0,
...
-- 7) Optional FK attachments (introspection; no assumptions)
do $do$
begin
  if exists (select 1 from information_schema.tables where table_schema='rgsr' and table_name='geometry_profiles')
     and exists (select 1 from information_schema.columns where table_schema='rgsr' and table_name='geometry_profiles' and column_name='geometry_profile_id') then
    begin
  ...
  if exists (select 1 from information_schema.tables where table_schema='rgsr' and table_name='geometry_elements')
     and exists (select 1 from information_schema.columns where table_schema='rgsr' and table_name='geometry_elements' and column_name='geometry_element_id') then
    begin
      execute 'alter table rgsr.geology_structures
...
-- 8) RLS + governance helpers (conservative)
...
create policy mp_select on rgsr.material_profiles for select to authenticated using (true);
drop policy if exists mp_write on rgsr.material_profiles;
create policy mp_write on rgsr.material_profiles for insert to authenticated with check (rgsr.is_sys_admin() or rgsr.is_service_role());
drop policy if exists mp_update on rgsr.material_profiles;
create policy mp_update on rgsr.material_profiles for update to authenticated using (rgsr.is_sys_admin() or rgsr.is_service_role()) with check (rgsr.is_sys_admin() or rgsr.is_service_role());
...
create policy gs_write on rgsr.geology_structures for insert to authenticated with check (rgsr.is_sys_admin() or rgsr.is_service_role());
drop policy if exists gs_update on rgsr.geology_structures;
create policy gs_update on rgsr.geology_structures for update to authenticated using (rgsr.is_sys_admin() or rgsr.is_service_role()) with check (rgsr.is_sys_admin() or rgsr.is_service_role());
...
create policy gl_write on rgsr.geology_layers for insert to authenticated with check (rgsr.is_sys_admin() or rgsr.is_service_role());
drop policy if exists gl_update on rgsr.geology_layers;
create policy gl_update on rgsr.geology_layers for update to authenticated using (rgsr.is_sys_admin() or rgsr.is_service_role()) with check (rgsr.is_sys_admin() or rgsr.is_service_role());
...
create policy wsi_write on rgsr.water_stone_interactions for insert to authenticated with check (rgsr.is_sys_admin() or rgsr.is_service_role());
drop policy if exists wsi_update on rgsr.water_stone_interactions;
create policy wsi_update on rgsr.water_stone_interactions for update to authenticated using (rgsr.is_sys_admin() or rgsr.is_service_role()) with check (rgsr.is_sys_admin() or rgsr.is_service_role());
...
create policy ss_write on rgsr.subsurface_features for insert to authenticated with check (rgsr.is_sys_admin() or rgsr.is_service_role());
drop policy if exists ss_update on rgsr.subsurface_features;
create policy ss_update on rgsr.subsurface_features for update to authenticated using (rgsr.is_sys_admin() or rgsr.is_service_role()) with check (rgsr.is_sys_admin() or rgsr.is_service_role());
...
create policy ggc_write on rgsr.geometry_geology_couplings for insert to authenticated with check (rgsr.is_sys_admin() or rgsr.is_service_role());
drop policy if exists ggc_update on rgsr.geometry_geology_couplings;
create policy ggc_update on rgsr.geometry_geology_couplings for update to authenticated using (rgsr.is_sys_admin() or rgsr.is_service_role()) with check (rgsr.is_sys_admin() or rgsr.is_service_role());
