...
begin
  if not exists (select 1 from pg_type t join pg_namespace n on n.oid=t.typnamespace
                 where n.nspname='engine_registry' and t.typname='artifact_kind') then
    create type engine_registry.artifact_kind as enum ('ENGINE_DEF','POLICY_DEF','RUN_INPUT','RUN_OUTPUT','RUN_META');
  end if;

  if not exists (select 1 from pg_type t join pg_namespace n on n.oid=t.typnamespace
                 where n.nspname='engine_registry' and t.typname='lane_class') then
    create type engine_registry.lane_class as enum ('PUBLIC','RESTRICTED','SENSITIVE','CLASSIFIED');
  end if;

  if not exists (select 1 from pg_type t join pg_namespace n on n.oid=t.typnamespace
                 where n.nspname='engine_registry' and t.typname='domain_class') then
...
do $$
begin
  if not exists (select 1 from pg_indexes where schemaname='engine_registry' and indexname='engines_code_version_ux') then
    create unique index engines_code_version_ux on engine_registry.engines(engine_code, version);
  end if;
end
$$;
...
do $$
begin
  if not exists (select 1 from pg_indexes where schemaname='engine_registry' and indexname='policies_code_version_ux') then
    create unique index policies_code_version_ux on engine_registry.policies(policy_code, version);
  end if;
end
$$;
...
do $$
begin
  if not exists (select 1 from pg_indexes where schemaname='engine_registry' and indexname='verticals_code_ux') then
    create unique index verticals_code_ux on engine_registry.verticals(vertical_code);
  end if;
end
$$;
...
do $$
begin
  if not exists (select 1 from pg_indexes where schemaname='engine_registry' and indexname='vertical_engines_ux') then
    create unique index vertical_engines_ux on engine_registry.vertical_engines(vertical_code, engine_code, engine_version);
  end if;
end
$$;
...
do $$
begin
  if not exists (select 1 from pg_indexes where schemaname='engine_registry' and indexname='vertical_policies_ux') then
    create unique index vertical_policies_ux on engine_registry.vertical_policies(vertical_code, policy_code, policy_version);
  end if;
end
$$;
...
do $$
begin
  if not exists (select 1 from pg_indexes where schemaname='engine_registry' and indexname='artifacts_kind_subject_ux') then
    create unique index artifacts_kind_subject_ux
